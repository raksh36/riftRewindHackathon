import { Trophy, Target, TrendingUp, Award, Crown, Zap, Star, Sword, Shield, Eye, Coins, Clock, Percent, Activity, Users, Flame } from 'lucide-react'
...
function StatsOverview({ stats, summonerName, enhancedAnalytics }) {
  ...
  const totalKills = (stats.avgKills || 0) * (stats.totalGames || 0)
  const totalDeaths = (stats.avgDeaths || 0) * (stats.totalGames || 0)
  const totalAssists = (stats.avgAssists || 0) * (stats.totalGames || 0)
  const killParticipation = stats.totalGames > 0 ? ((totalKills + totalAssists) / Math.max(totalKills + totalDeaths + totalAssists, 1) * 100).toFixed(1) : 0
  
  // Game duration estimate (assume 25-30 min average)
  const avgGameDuration = 27
  const totalHoursPlayed = ((stats.totalGames || 0) * avgGameDuration / 60).toFixed(1)
  ...
  const primaryStatCards = [
    {
      ...
      subValue: `${totalHoursPlayed}hrs played`,
      color: 'text-blue-400',
    ...
    },
    {
      ...
      subValue: `${stats.wins || 0}W / ${(stats.totalGames || 0) - (stats.wins || 0)}L`,
    ...
    },
    {
      ...
      subValue: `${(stats.avgKills || 0).toFixed(1)}/${(stats.avgDeaths || 0).toFixed(1)}/${(stats.avgAssists || 0).toFixed(1)}`,
    ...
    },
    {
      ...
      subValue: `${((stats.topChampions?.[0]?.games || 0) / Math.max(stats.totalGames || 1, 1) * 100).toFixed(0)}% of games`,
  ...
  const secondaryStatCards = [
    {
      ...
      value: (stats.avgKills || 0).toFixed(1),
      color: 'text-red-400'
    },
    {
      icon: <Shield className="w-5 h-5" />,
      label: 'Deaths per Game',
      value: (stats.avgDeaths || 0).toFixed(1),
      color: 'text-gray-400'
    },
    {
      icon: <Users className="w-5 h-5" />,
      label: 'Assists per Game',
      value: (stats.avgAssists || 0).toFixed(1),
      color: 'text-blue-400'
    },
    {
    ...
    },
    {
      ...
      label: 'CS per Minute',
      value: ((stats.avgCS || 0) / avgGameDuration).toFixed(1),
      color: 'text-yellow-400'
    },
    {
      icon: <Activity className="w-5 h-5" />,
      label: 'Damage Share',
      value: `${((stats.avgDamage || 0) / 20000 * 100).toFixed(0)}%`,
      color: 'text-orange-400'
    },
    {
      icon: <Eye className="w-5 h-5" />,
      label: 'Vision Score/Min',
      value: ((stats.avgVisionScore || 0) / avgGameDuration).toFixed(1),
      color: 'text-cyan-400'
    },
    {
      icon: <Flame className="w-5 h-5" />,
      label: 'Multi-kills',
      value: (stats.achievements?.doubleKills || 0) + (stats.achievements?.tripleKills || 0),
      color: 'text-red-500'
    }
  ]
  ...
  return (
    <div className="space-y-6">
      {/* Hero Stats */}
      <div className="card bg-gradient-to-br from-rift-blue/20 to-rift-purple/20 text-center animate-fade-in">
        <h2 className="text-4xl font-bold gradient-text mb-2">{summonerName}'s Performance</h2>
        <p className="text-gray-300 mb-6 text-lg">Complete Season Overview</p>
        
        {/* Live Status Badge */}
        {liveStatus?.in_game && (
          <div className="mb-6 inline-flex items-center gap-2 px-4 py-2 bg-green-500/20 border border-green-500/50 rounded-full glow-pulse">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
            <span className="text-green-400 font-semibold">Currently In Game</span>
          </div>
        )}
        
        {/* Primary Stats - 4 Main Cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {primaryStatCards.map((card, index) => (
            <div
              key={index}
              className={`stat-card bg-gradient-to-br ${card.bgColor} hover-lift group`}
        ...
        {/* Secondary Stats Grid - 8 Detailed Metrics */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-4 border-t border-gray-700/50">
          {secondaryStatCards.map((card, index) => (
            <div
              key={index}
              className="p-3 bg-gray-800/40 rounded-lg border border-gray-700/30 hover:border-gray-600/50 transition-all hover:scale-105"
            >
      ...
      {/* Champion Pool & Playstyle */}
      {(stats.topChampions && stats.topChampions.length > 0) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Champion Pool Diversity */}
          <div className="card bg-gradient-to-br from-cyan-900/20 to-blue-900/20 border border-cyan-500/30">
            <h3 className="text-xl font-bold text-white mb-4 flex items-center">
              <Trophy className="w-5 h-5 mr-2 text-cyan-400" />
              Champion Pool
            </h3>
            <div className="space-y-3">
              {stats.topChampions.slice(0, 5).map((champ, idx) => (
                <div key={idx} className="flex items-center justify-between p-2 bg-cyan-900/20 rounded-lg">
                  <div className="flex items-center gap-3">
                    <div className="text-2xl font-bold text-cyan-400">#{idx + 1}</div>
                    <div>
                      <div className="text-white font-semibold">{champ.name}</div>
                      <div className="text-xs text-gray-400">{champ.games} games • {champ.winRate}% WR</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className={`text-lg font-bold ${getKDAColor(champ.kda)}`}>
            ...
            <div className="mt-4 pt-4 border-t border-cyan-500/30 text-center">
              <div className="text-sm text-gray-400">
                Pool Diversity: <span className="font-bold text-cyan-400">
                  {stats.topChampions.length} Champions
                </span>
              </div>
            </div>
          </div>
          ...
          {/* Playstyle Insights */}
          <div className="card bg-gradient-to-br from-pink-900/20 to-purple-900/20 border border-pink-500/30">
            <h3 className="text-xl font-bold text-white mb-4 flex items-center">
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between mb-2">
                  ...
                  <span className="text-sm font-bold text-pink-400">
                    {((stats.avgKills || 0) / Math.max(stats.avgDeaths || 1, 1) * 33).toFixed(0)}%
                  </span>
                </div>
                <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-pink-500 to-red-500 transition-all duration-500"
                    style={{ width: `${Math.min(((stats.avgKills || 0) / Math.max(stats.avgDeaths || 1, 1) * 33), 100)}%` }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-gray-400">Vision Control</span>
                  <span className="text-sm font-bold text-yellow-400">
                    {Math.min(((stats.avgVisionScore || 0) / 50 * 100), 100).toFixed(0)}%
                  </span>
                </div>
                <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-yellow-500 to-orange-500 transition-all duration-500"
                    style={{ width: `${Math.min(((stats.avgVisionScore || 0) / 50 * 100), 100)}%` }}
                  />
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between mb-2">
                  <span className="text-sm text-gray-400">Farming</span>
                  <span className="text-sm font-bold text-green-400">
                    {Math.min(((stats.avgCS || 0) / 200 * 100), 100).toFixed(0)}%
                  </span>
                </div>
                <div className="h-3 bg-gray-800 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-green-500 to-emerald-500 transition-all duration-500"
                    style={{ width: `${Math.min(((stats.avgCS || 0) / 200 * 100), 100)}%` }}
                  />
                </div>
              </div>
            </div>
            ...
            {/* Playstyle Summary */}
            <div className="mt-4 pt-4 border-t border-pink-500/30 p-3 bg-pink-900/20 rounded-lg">
              <div className="text-sm text-gray-300 text-center">
                {stats.avgKills > 7 ? '⚔️ Aggressive Carry' : stats.avgDeaths < 4 ? '🛡️ Calculated Player' : stats.avgAssists > 10 ? '🤝 Team-Oriented' : '⚖️ Balanced'} • {stats.avgCS > 180 ? '🌾 Farm-Focused' : '💥 Fight-Oriented'}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Epic Achievements */}
      {stats.achievements && (stats.achievements.pentakills > 0 || stats.achievements.quadrakills > 0 || stats.achievements.tripleKills > 0 || stats.achievements.doubleKills > 0) && (
        <div className="card bg-gradient-to-r from-yellow-900/20 to-orange-900/20 border border-yellow-500/30">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {(stats.achievements?.pentakills || 0) > 0 && (
              <div className="text-center p-4 bg-yellow-900/30 rounded-lg hover-lift">
            ...
            {(stats.achievements?.quadrakills || 0) > 0 && (
              <div className="text-center p-4 bg-orange-900/30 rounded-lg hover-lift">
                <div className="text-5xl mb-2">⚡</div>
            ...
            {(stats.achievements?.tripleKills || 0) > 0 && (
              <div className="text-center p-4 bg-red-900/30 rounded-lg hover-lift">
                <div className="text-5xl mb-2">🔥</div>
                <div className="text-3xl font-bold text-red-400">
                  {stats.achievements.tripleKills}
                </div>
                <div className="text-sm text-gray-400">Triple Kills</div>
              </div>
            )}
            {(stats.achievements?.doubleKills || 0) > 0 && (
              <div className="text-center p-4 bg-pink-900/30 rounded-lg hover-lift">
                <div className="text-5xl mb-2">💥</div>
                <div className="text-3xl font-bold text-pink-400">
                  {stats.achievements.doubleKills}
                </div>
                <div className="text-sm text-gray-400">Double Kills</div>
              </div>
            )}
          </div>
        </div>
